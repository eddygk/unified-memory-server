...
class TestMemorySelectorConfig(unittest.TestCase):
    ...
    def test_client_getters(self):
        ...
        # Test that _get_redis_client raises ValueError when REDIS_URL not configured
        with self.assertRaises(ValueError):
            selector._get_redis_client()
        
        # Test that _get_basic_memory_client returns None (placeholder)
        basic_client = selector._get_basic_memory_client()
        self.assertIsNone(basic_client)
        
        # Test that _get_neo4j_client raises ValueError when Neo4j URL or password not configured
        with self.assertRaises(ValueError):
